				...
				<ul class="navbar-nav mr-auto">
					@if (!$settings->disable_creators_section)
						...
						<form class="form-inline my-lg-0 position-relative" method="get" action="{{ url('creators') }}">
							<input id="searchCreatorNavbar" class="form-control search-bar @if(auth()->guest() && request()->path() == '/') border-0 @endif" type="text" required name="q" autocomplete="off" minlength="3" placeholder="{{ __('general.find_user') }}" aria-label="Search">
				...
				<ul class="navbar-nav ml-auto">
					...
					<li class="nav-item dropdown d-lg-none">
						<a href="{{ auth()->user()->dark_mode == 'off' ? url('mode/dark') : url('mode/light') }}" class="nav-link px-2 link-menu-mobile py-1">
							<div>
								<i class="bi bi-{{ auth()->user()->dark_mode == 'off' ? 'moon-fill' : 'sun-fill' }} mr-2"></i>
					...
					<li class="nav-item dropdown d-lg-block d-none">
						<a class="nav-link px-2" href="{{ url('/') }}" title="{{ __('admin.home') }}">
							<i class="bi bi-house-door-fill icon-navbar"></i>
					...
					<li class="nav-item dropdown d-lg-block d-none">
						<a class="nav-link px-2" href="{{ url('creators') }}" title="{{ __('general.explore_creators') }}">
							<!-- Se usa un mapamundi (icono 'bi-globe') en lugar de una brújula -->
							<i class="bi bi-globe icon-navbar"></i>
					...
					<li class="nav-item dropdown d-lg-block d-none">
						<a class="nav-link px-2" href="{{ url('shop') }}" title="{{ __('general.shop') }}">
							<i class="bi bi-shop icon-navbar"></i>
					...
					<li class="nav-item dropdown d-lg-block d-none">
						<a href="{{ url('messages') }}" class="nav-link px-2" title="{{ __('general.messages') }}">
							...
							<!-- Se usa un globo de mensajes (icono 'bi-chat-dots') en lugar del icono anterior -->
							<i class="bi bi-chat-dots icon-navbar"></i>
					...
					<li class="nav-item dropdown d-lg-block d-none">
						<a class="nav-link" href="#" id="nav-inner-success_dropdown_1" role="button" data-toggle="dropdown">
							...
							<i class="bi bi-caret-down-fill m-0 align-middle"></i>
						</a>
						<div class="dropdown-menu mb-1 dropdown-menu-right dd-menu-user" aria-labelledby="nav-inner-success_dropdown_1">
							...
							<a class="dropdown-item dropdown-navbar" href="{{ url('panel/admin') }}">
								<i class="bi bi-speedometer2 mr-2"></i> {{ __('admin.admin') }}
							</a>
							...
							<span class="dropdown-item dropdown-navbar balance">
								<i class="bi bi-currency-dollar mr-2"></i> {{ __('general.balance') }}: {{ Helper::amountFormatDecimal(auth()->user()->balance) }}
							</span>
							@endif
							...
							@if ($settings->disable_wallet == 'on' && auth()->user()->wallet != 0.00 || $settings->disable_wallet == 'off')
								...
								<a class="dropdown-item dropdown-navbar" href="{{ url('my/wallet') }}">
									<i class="bi bi-wallet2 mr-2"></i> {{ __('general.wallet') }}:
							...
							<a class="dropdown-item dropdown-navbar url-user" href="{{ url(auth()->User()->username) }}">
								<i class="bi bi-person-circle mr-2"></i> {{ auth()->user()->verified_id == 'yes' ? __('general.my_page') : __('users.my_profile') }}
							</a>
							...
							<a class="dropdown-item dropdown-navbar" href="{{ url('my/posts') }}">
								<i class="bi bi-pencil-square mr-2"></i> {{ __('general.my_posts') }}
							</a>
							@endif
							...
							@if (auth()->user()->verified_id == 'yes')
							<a class="dropdown-item dropdown-navbar" href="{{ url('my/subscribers') }}">
								<i class="bi bi-people mr-2"></i> {{ __('users.my_subscribers') }}
							</a>
							@endif

							<a class="dropdown-item dropdown-navbar" href="{{ url('my/subscriptions') }}">
								<i class="bi bi-person-check mr-2"></i> {{ __('users.my_subscriptions') }}
							</a>
							<a class="dropdown-item dropdown-navbar" href="{{ url('my/bookmarks') }}">
								<i class="bi bi-bookmark mr-2"></i> {{ __('general.bookmarks') }}
							</a>
							<a class="dropdown-item dropdown-navbar" href="{{ url('my/likes') }}">
								<i class="bi bi-heart mr-2"></i> {{ __('general.likes') }}
							</a>

							@if (auth()->user()->verified_id == 'no' && auth()->user()->verified_id != 'reject' && $settings->requests_verify_account == 'on')
							<div class="dropdown-divider"></div>
							<a class="dropdown-item dropdown-navbar" href="{{ url('settings/verify/account') }}">
								<i class="bi bi-star-fill mr-2"></i> {{ __('general.become_creator') }}
							</a>
							@endif
							...
							<a class="dropdown-item dropdown-navbar" href="{{ url('mode/dark') }}">
								<i class="bi bi-moon-fill mr-2"></i> {{ __('general.dark_mode') }}
							</a>
							@else
							<a class="dropdown-item dropdown-navbar" href="{{ url('mode/light') }}">
								<i class="bi bi-sun-fill mr-2"></i> {{ __('general.light_mode') }}
							</a>
							@endif

							<div class="dropdown-divider dropdown-navbar"></div>
							<a class="dropdown-item dropdown-navbar" href="{{ url('logout') }}">
								<i class="bi bi-box-arrow-right mr-2"></i> {{ __('auth.logout') }}
							</a>
						</div>
					</li>
